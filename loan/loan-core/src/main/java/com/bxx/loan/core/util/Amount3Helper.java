...
public class Amount3Helper {
     ...
     * @return 每月偿还利息
     */
    public static BigDecimal getOneMonthInterest(BigDecimal invest, BigDecimal yearRate) {
        //每月偿还利息
        BigDecimal monthIncome = invest.multiply(yearRate).divide(new BigDecimal(12), 8, BigDecimal.ROUND_DOWN);
        return monthIncome;
    }
     ...
     * @return 每月偿还本金
     */
    public static BigDecimal getOneMonthPrincipal(BigDecimal invest, int totalMonth, int curPeriod) {
        // 每月利息
        if(curPeriod == totalMonth){
            return invest;
        } else {
